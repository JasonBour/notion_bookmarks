...
import React from 'react'; // 确保导入 React 用于类型声明
...
const IconRenderer: React.FC<IconRendererProps> = ({ iconName, className }) => {
    ...
    // ⚠️ 修复：明确指定动态导入组件的类型为 React.ComponentType<{ className: string }>，解决类型错误
    const LucideIcon = dynamic<React.ComponentType<{ className: string }>>(() =>
        // 尝试从 lucide-react 模块中获取对应名称的组件
        import('lucide-react').then((mod) => {
            // 类型断言，确保我们获取的是一个 React 组件
            const Component = mod[iconName as keyof typeof mod] as React.ComponentType<{ className: string }> | undefined;
    ...
    // 此时 TypeScript 知道 LucideIcon 可以接受 className prop
